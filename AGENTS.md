...
- Docker: `make start` / `make stop` / `make smoke`. `make smoke` needs the production stack (`make build`, then `docker compose -f compose.yaml up -d --wait --wait-timeout 60`) and refuses the dev stack: it stops PHP, runs `down`/`up` (volumes kept) and leaves the production stack running. Plain `docker compose` also loads `compose.override.yaml` (dev target, mounted code); drive the production stack with `docker compose -f compose.yaml …` (`make build` builds its image).
