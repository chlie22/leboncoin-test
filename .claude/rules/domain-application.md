...
- Use cases: one class, one public `execute()`. `GenerateFizzBuzz` calls `record()` exactly once. On `StatisticsStoreUnavailable` it logs `statistics.record_skipped` at warning level with the error class, never the parameters, and still returns the sequence. No retry.
