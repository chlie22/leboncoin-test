...
- Entrypoint (`set -eu`, stops at the first error):
  - Only when the command is `php-fpm`: `php bin/console doctrine:migrations:migrate --no-interaction`, then `php bin/console app:statistics:apply-window`, then `exec "$@"`.
