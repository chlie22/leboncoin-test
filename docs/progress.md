...
**Étape 6** : Application en TDD : port `RequestStatisticsStore`, `GenerateFizzBuzz`, `GetMostFrequentRequest`, `Model\RequestStatistics`, `StatisticsStoreUnavailable`, `tests/Support/InMemoryRequestStatisticsStore` (mode « en panne ») et suite de contrat abstraite ; tests unitaires et de contrat verts, sans I/O.

- Sections de la spec à lire : §3.4, §5.3, §5.4, §5.10, §6.3 (invariants du contrat), §9.1 (lignes Application et Contrat), §10 ; rules `domain-application.md` et `testing.md`.
- `==` entre deux `FizzBuzzParameters` est lâche (`'0' == '00'` vaut `true`) : l'adaptateur en mémoire compare ou indexe strictement ; `equals()` s'ajoute seulement s'il a un appelant.
- `config/services.yaml` : exclure `Domain/FizzBuzzParameters.php`, `Domain/Exception/` et `Application/Model/`, **pas** tout `Domain/`, sinon `FizzBuzzGenerator` n'est plus injectable ; préciser §10 (« exclusion Domain / Model »).
- `tests/Contract/` n'est dans aucune suite PHPUnit : la classe abstraite n'est exécutée que par ses sous-classes. Logger de test : vérifier si `psr/log` fournit encore `Psr\Log\Test\TestLogger` (sinon une petite classe dans `tests/Support/`).
- Deptrac n'analyse que `src/` : « sans I/O » dans les tests se vérifie à la lecture. En CI, Deptrac est muet quand tout est propre.
